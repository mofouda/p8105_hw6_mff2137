...
First, we load and clean the data for regression analysis. The variables `babysex`, `frace`, `malform`, `mrace` were converted to factor in our dataset. 
...
In this step, we compare our model to two others:
* One using length at birth and gestational age as predictors (main effects only)
* One using head circumference, length, sex, and all interactions (including the three-way interaction) between these

We utilize `cross_cv` function for cross-validating different models and `map` and `map2_dbl` from `purr` package to iterate across `test` and `train` data frames. The resulting `rmse` is plotted against our 3 models to visually assess model fit and performance. 
